...
t=-0.04:ts:0.04;
Ta=0.01;      % variable 
m_sig=Rect((t-0.01)/0.01)+Rect((t+0.01)/0.01);
Lm_sig=length(m_sig);
...
freqm=(-Lfft/2:(Lfft/2)-1)/(Lfft*ts) % Defining the frequency axis for the frequency domain DSB modulated signal  -> normalize krne k lyai FFT use krte hain 
B_m=150;
